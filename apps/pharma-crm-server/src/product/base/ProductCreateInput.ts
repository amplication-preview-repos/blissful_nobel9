/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  IsNumber,
  Min,
  Max,
  ValidateNested,
} from "class-validator";
import { PurchaseCreateNestedManyWithoutProductsInput } from "./PurchaseCreateNestedManyWithoutProductsInput";
import { Type } from "class-transformer";
import { QuotationCreateNestedManyWithoutProductsInput } from "./QuotationCreateNestedManyWithoutProductsInput";
import { SaleCreateNestedManyWithoutProductsInput } from "./SaleCreateNestedManyWithoutProductsInput";

@InputType()
class ProductCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsNumber()
  @Min(-999999999)
  @Max(999999999)
  @IsOptional()
  @Field(() => Number, {
    nullable: true,
  })
  price?: number | null;

  @ApiProperty({
    required: false,
    type: () => PurchaseCreateNestedManyWithoutProductsInput,
  })
  @ValidateNested()
  @Type(() => PurchaseCreateNestedManyWithoutProductsInput)
  @IsOptional()
  @Field(() => PurchaseCreateNestedManyWithoutProductsInput, {
    nullable: true,
  })
  purchases?: PurchaseCreateNestedManyWithoutProductsInput;

  @ApiProperty({
    required: false,
    type: () => QuotationCreateNestedManyWithoutProductsInput,
  })
  @ValidateNested()
  @Type(() => QuotationCreateNestedManyWithoutProductsInput)
  @IsOptional()
  @Field(() => QuotationCreateNestedManyWithoutProductsInput, {
    nullable: true,
  })
  quotations?: QuotationCreateNestedManyWithoutProductsInput;

  @ApiProperty({
    required: false,
    type: () => SaleCreateNestedManyWithoutProductsInput,
  })
  @ValidateNested()
  @Type(() => SaleCreateNestedManyWithoutProductsInput)
  @IsOptional()
  @Field(() => SaleCreateNestedManyWithoutProductsInput, {
    nullable: true,
  })
  sales?: SaleCreateNestedManyWithoutProductsInput;
}

export { ProductCreateInput as ProductCreateInput };
